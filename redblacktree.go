...
func (tree *Tree) Put(key KeyType, value ValueType) {
...
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (tree *Tree) Get(key KeyType) (value ValueType, found bool) {
...
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (tree *Tree) Remove(key KeyType) {
	var child *Node
	node := tree.lookup(key)
	if node == nil {
		return
...
func (tree *Tree) Values() []ValueType {
...
//
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (tree *Tree) Floor(key KeyType) (floor *Node, found bool) {
	found = false
	node := tree.Root
	for node != nil {
		compare := tree.Comparator(key, node.Key)
...
//
// Key should adhere to the comparator's type assertion, otherwise method panics.
func (tree *Tree) Ceiling(key KeyType) (ceiling *Node, found bool) {
	found = false
	node := tree.Root
	for node != nil {
		compare := tree.Comparator(key, node.Key)
...
func (tree *Tree) GetMax() (node *Node, found bool) {
...
func (tree *Tree) lookup(key KeyType) *Node {
	node := tree.Root
	for node != nil {
		compare := tree.Comparator(key, node.Key)
		switch {
...
func (iterator *Iterator) Value() ValueType {
	return iterator.node.Value
}

// Key returns the current element's key.
// Does not modify the state of the iterator.
func (iterator *Iterator) Key() KeyType {
